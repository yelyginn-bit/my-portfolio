...
const LEGAL_OPERATOR = {
  name: "[ФИО ИП / Наименование оператора]",
  inn: "[ИНН]",
  ogrn: "[ОГРН / ОГРНИП]",
...
const PrivacyContent = () => (
  <div className="max-w-4xl">
    ...
    <div className="mt-8 space-y-4 text-sm sm:text-base leading-relaxed text-nothing-black/85">
      <p>
        Документ составлен в соответствии с Федеральным законом от 27.07.2006 N 152-ФЗ «О персональных данных» и
        определяет порядок обработки персональных данных и меры по обеспечению их безопасности.
      </p>
      <p>
        <strong>Оператор персональных данных:</strong> {LEGAL_OPERATOR.name}
        <br />
        <strong>ИНН:</strong> {LEGAL_OPERATOR.inn}
        <br />
        <strong>ОГРН / ОГРНИП:</strong> {LEGAL_OPERATOR.ogrn}
        <br />
        <strong>Контакты для обращений:</strong> {LEGAL_OPERATOR.contact}
      </p>
      <p>
        <strong>Какие данные обрабатываются:</strong> имя, контакт для обратной связи (email / мессенджер / телефон),
        текст сообщения в форме, а также технические данные (cookie / localStorage), необходимые для работы сайта.
      </p>
      <p>
        <strong>Цели обработки:</strong> обработка входящих заявок, обратная связь по запросу пользователя, ведение
        коммуникации по проекту, техническое обеспечение работы сайта.
      </p>
      <p>
        <strong>Правовые основания:</strong> согласие субъекта персональных данных, а также иные основания,
        предусмотренные законодательством РФ.
      </p>
      <p>
        <strong>Передача третьим лицам:</strong> персональные данные не передаются третьим лицам, кроме случаев,
        предусмотренных законом, либо когда это необходимо для ответа пользователю через выбранный им канал связи.
      </p>
      <p>
        <strong>Трансграничная передача:</strong> на сайте используются внешние сервисы (например, Telegram и видеосервисы
        для встраивания контента). При взаимодействии с ними возможна передача данных на территорию иностранных государств.
      </p>
      <p>
        <strong>Срок хранения:</strong> данные хранятся не дольше, чем требуется для достижения целей обработки, либо до
        отзыва согласия, если иное не установлено законом.
      </p>
      <p>
        <strong>Права субъекта данных:</strong> пользователь вправе запросить сведения об обработке своих данных,
        потребовать их уточнения, блокирования, удаления, а также отозвать согласие на обработку.
      </p>
      <p>
        <strong>Cookie:</strong> сайт использует cookie / localStorage для корректной работы интерфейса. Пользователь
        может ограничить использование cookie в настройках браузера.
      </p>
      <p>
        По вопросам обработки персональных данных направляйте обращение по контакту оператора: {LEGAL_OPERATOR.contact}.
      </p>
    </div>
  </div>
);
...
const PortfolioGrid = () => {
  ...
  return (
    <section id="projects" className="portfolio-section pt-14 pb-20 md:pt-16 md:pb-24 px-4 sm:px-6 md:px-10 xl:px-12 max-w-[1720px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 sm:gap-12 mb-12 sm:mb-24">
        ...
        <div className="flex flex-nowrap md:flex-wrap gap-2 sm:gap-4 overflow-x-auto md:overflow-x-visible pb-4 md:pb-0 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
          {PORTFOLIO_DATA.map((cat) => (
            <button
              ...
              className={cn(
                "px-4 sm:px-6 py-2 sm:py-3 rounded-full font-mono text-[10px] sm:text-xs uppercase tracking-widest transition-all border whitespace-nowrap",
...
export default function App() {
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-nothing-white selection:bg-nothing-red selection:text-nothing-white overflow-x-hidden">
      ...
      {/* Navigation */}
      {renderNav("site-nav fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 px-4 sm:px-6 lg:px-8 py-2 sm:py-3 flex justify-between items-center glass rounded-full w-[calc(100%-1rem)] sm:w-[95%] max-w-[1720px] border-white/20 lg:hidden")}
      ...
      {/* Services Section */}
      <section id="services" className="services-section pt-16 pb-20 md:pt-20 md:pb-28 px-6 md:px-10 xl:px-12 bg-nothing-gray/30 relative overflow-hidden">
            ...
            <motion.div variants={STAGGER_ITEM} className="flex flex-col gap-4 sm:gap-8 mt-12 sm:mt-16">
              ...
              <a href="https://t.me/YuriElygin" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-4 sm:gap-8 p-6 sm:p-8 hardware-border rounded-[1.5rem] sm:rounded-[2rem] hover:bg-nothing-black hover:text-nothing-white transition-all duration-500 bg-nothing-white">
                ...
                <div>
                  ...
                  <p className="text-lg sm:text-xl font-bold">@YuriElygin</p>
                </div>
              </a>
            </motion.div>
          </motion.div>
            ...
            <div className="relative z-10">
              ...
              <form className="space-y-3.5 sm:space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                <motion.button 
                  ...
                  whileTap={{ scale: 0.98 }}
                  disabled={!privacyAccepted || !personalDataAccepted}
      ...
      {/* Footer */}
      <footer className="site-footer py-20 md:py-32 px-6 md:px-10 xl:px-12 border-t border-nothing-black/10 bg-nothing-white transition-colors">
          ...
          <div className="flex justify-center items-center pt-12 border-t border-nothing-black/10">
            <div className="text-center">
              ...
              <p className="mt-2 text-[11px] sm:text-xs text-nothing-black/60">
                Оператор: {LEGAL_OPERATOR.name}. ИНН: {LEGAL_OPERATOR.inn}. ОГРН/ОГРНИП: {LEGAL_OPERATOR.ogrn}.
              </p>
            </div>
          </div>
        </div>
